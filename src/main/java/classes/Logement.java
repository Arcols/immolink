...
public class Logement extends BienLouable {
	...
	public Logement(int nb_piece, double surface, String numero_fiscal, String ville, String adresse,
		...
		this.insertIntoTable(nb_piece, surface, numero_fiscal, ville, adresse, complement_adresse, diagnostic,
				haveGarage);
	}
	...
	private void insertIntoTable(int nb_piece, double surface, String numero_fiscal, String ville, String adresse,
		...
		pstmt.setInt(6, this.foundIDBatInDB(db, ville, adresse));
		if (haveGarage) {
			pstmt.setInt(7, this.getIdGarageAssocié(db, numero_fiscal));
		}
		pstmt.executeUpdate();
		pstmt.close();
		int idBien = this.getIdBien(db, numero_fiscal);
		for (Diagnostic diagnostic : diagnostics) {
			this.addDiagnosticToBd(db, idBien, diagnostic);
		}
		db.closeConnection();
	}
